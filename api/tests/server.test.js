...
describe('Transaction end-points', () => {
    describe('[GET] invalid url', () => {
        it('returns Here to fetch reward points? Please navigate to a valid endpoint.', async () => {
            const res = await request(server).get('/foo/bar')
            expect(res.body.message).toBe('Here to fetch reward points? Please navigate to a valid endpoint.')
        })
  })
...
describe('[GET] /api/transactions/points/payer', () => {
    it('returns 200 OK status', async () => {
        const res = await request(server).get('/api/transactions/points/payer')
        expect(200)
    })
    it('returns JSON', async () => {
        const res = await request(server).get('/api/transactions/points/payer')
...
describe('[POST] /api/transactions/points/add', () => {
...
describe('[POST] /api/transactions/points/spend', () => {
    it('responds with a validation message if points is missing from request', async () => {
        const res = await request(server)
